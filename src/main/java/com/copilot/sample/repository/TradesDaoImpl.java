...
//				new Object[] { Integer.valueOf(trades.getTradeId()), "EXIT" }, rs -> {
//					if (rs.next()) {
...
//				new Object[] { Integer.valueOf(trades.getTradeId()), "NEW-TRADE" },
...
//		}
		...
		// Add custom query to create a new trade
		jdbcTemplate.update(
				"insert into trades (transactionType, buyer, seller, amount, currency, tradeDate, status) values(?,?,?,?,?,?,?)",
				trades.getTransaction(), trades.getBuyer(), trades.getSeller(), Integer.valueOf(trades.getAmount()),
